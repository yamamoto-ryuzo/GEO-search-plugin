...
地図検索で検索結果を選んだときの移動（パン）挙動を分かりやすくするため、以下の5つの移動モードを追加しました。ダイアログの「パンモード」から切り替えて使えます。

- ズームして選択範囲にフィット
  - 選択した地物全体が画面に収まるように自動でズームして表示します。地物の形状全体を確認したいときに便利です。
- 中心へパン（ズーム維持）
  - 現在の縮尺をそのままに、地物の中心位置が画面中央に来るように平滑に移動（パン）します。縮尺を変えたくない場合に使います。
- 固定縮尺で表示
  - ダイアログ左側の縮尺入力欄で指定した固定の縮尺で表示します（入力はドロップダウンまたは直接数値入力が可能）。印刷イメージや等尺条件で見たい場合に使用します。
- アニメーションパン
  - 滑らかなアニメーションで現在位置から地物の位置へ移動します。視点移動がわかりやすく、複数回の移動時に位置関係を追いやすいモードです。
- 選択のみ（ビュー変更なし）
  - 地物を選択するだけでマップの表示は変えません。位置はそのままに属性や関連情報を確認したいときに使います。

補足:
- 「パンモード」は検索ダイアログ内の「パンモード」ドロップダウンで選択できます。固定縮尺の値はその左にある縮尺入力欄（数値入力可）で設定してください。
