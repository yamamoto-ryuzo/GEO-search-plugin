...
- 「selectTheme」欄で、QGISプロジェクト内の任意のマップテーマを選択できます：
  ...
  - 編集内容はプロジェクト変数「geo_search」または `setting.json` に反映されます。

> 詳細な設定内容やフィールドの意味は下記「設定ファイル説明」も参照してください。
{
  "SearchTabs": [
 
ここ以下をプロジェクト変数「geo_search」もしくはプラグイン内の設定ファイル「setting.json」に設定してください。
　・プロジェクト変数「geo_search」　　　プロジェクトごとに変更したい検索  
