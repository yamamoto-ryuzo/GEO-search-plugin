...
## User-theme（ユーザーテーマ） — 開発中

このプラグインには「ユーザーテーマ」と呼ぶ最小限の表示スナップショット機能があります。
現在は開発中の機能であり、目的は「ユーザーが意図した可視化の最低限（可視レイヤ・凡例の可視項目・可能ならスタイル名）を保存して復元する」ことです。

注意: この機能はプロジェクト全体を完全に復元するものではありません（.qgs を丸ごと再生成するわけではありません）。レンダラーや QGIS バージョン差により復元が完全でない場合があります。

- 保存形式: JSON（推奨拡張子例: `.usertheme.json`）
- 保存内容（概略）:
  - `meta`: 生成時のメタ情報（version, timestamp, project filepath など）
  - `entries`: 可視レイヤの配列（各要素に `order`, `id`, `name`, `style`, `legend_items` ）
...
適用の方針（ベストエフォート）:
- レイヤはまず `id` で検索し、見つからなければ `name` でフォールバックします。名前が重複する場合は最初に一致したレイヤに適用します。
- `style` が指定されていれば `_apply_layer_style_by_name`（`geo_search/theme.py` にあるベストエフォート実装）で適用を試みます。
- `legend_items` は `label` を手掛かりにレンダラー内部の項目を探し、可視化フラグを適用します。カテゴリレンダラーでは保存時の値を分解してマッチングを試みます。

既知の制限（開発中）:
- 完全な互換性は保証しません。特に複雑なルールベースレンダラーや QGIS の大幅なバージョン差では期待通りに復元できないことがあります。
- スタイルの適用は名前/ID ベースの単純適用に依存するため、同名のスタイルが存在しない場合は適用されません。
- レイヤ名重複時の挙動は単純なフォールバック（最初の一致）です。

テスト手順（開発者向け簡易チェックリスト）:
1. QGIS でプロジェクトを保存しておく。
2. 設定ダイアログから `Save User Theme...` を選び、`project/themes/` 等へ保存する。
3. 保存された JSON を確認する（`entries` が適切に入っているか）。
4. プロジェクト内でレイヤ状態を意図的に変える（可視/非可視、別スタイル適用など）。
5. `Load User Theme...` を実行して復元動作を確認する。
6. QGIS のメッセージログ（タグ: `GEO-search-plugin`）を確認し、どのレイヤに適用できたか／失敗したかを確認する。

実装上の注意点（メンテ向け）:
- スナップショット収集は `collect_visible_layer_snapshot`、適用は `apply_user_theme`（`geo_search/theme.py`）を使います。
- 永続化はファイル（JSON）に任せていますが、必要なら将来プロジェクト変数へパスを保存する仕組みを追加できます。
- このセクションは開発中の仕様を説明するためのものです。ユーザー向けの最終ドキュメント化は安定版が確定してから行ってください。
