...
3. 選択テーマ適用後の「可視レイヤ一覧」と各レイヤの凡例状態（legend）をメモリにスナップショットとして保存します（`tmp_sel` 相当）。実装上は `collect_visible_layer_messages(..., snapshot_name=...)` を使います。
...
### 実装上の主要関数（参照）
- `collect_visible_layer_messages(root, log_layer_legend_state, snapshot_name=...)` — 可視レイヤ一覧と凡例状態をメモリにスナップショット保存します。既存のログ出力も行います。
